...
highfink::highfink(const highfink &h) : abstr_emp0(h), manager(h), fink(h) {}//菱形继承最后一个派生类的构造函数方法是前面构造函数都需要使用。
void highfink::ShowAll()const//基类的返回值及重新设置是为了派生类能够更改基类的私有成员。
{
	abstr_emp0::ShowAll();
